# Declarative disk layout for a from-scratch install, applied with disko
...
# subvolumes this machine already had (root/home/nix/log/persist) plus a
...
#
# The disk is addressed by its stable /dev/disk/by-id path (not /dev/nvme0n1)
...
# running disko — this id is specific to this machine's Kingston NVMe drive.
{
  disko.devices = {
    disk = {
      main = {
          ...
          partitions = {
            ...
            luks = {
              ...
              content = {
                type = "luks";
                name = "crypted";
                # No passwordFile/keyFile set on purpose: disko prompts for
                # the passphrase interactively while partitioning, and with
                # no keyFile carried into the resulting config, every future
                # boot prompts for it too via systemd-boot's initrd — that's
                # the actual encryption, not just an install-time step.
                settings = {
