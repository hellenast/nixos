...
If `protonvpn.nix`/`secrets.nix` are already deployed and I'm just swapping in a new config, skip straight to step 4 — no rebuild needed, just re-copy `secrets.yaml` and restart the service.
...
Use the "<Name> (VPN)" entries in my app launcher/rofi (e.g. "Vesktop (VPN)") instead of the plain ones — those are the ones wired through `protonvpn-run`. Which apps get one is controlled by the `apps` list near the top of `protonvpn.nix`.
