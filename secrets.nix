...
{
  # sops-nix: decrypts secrets/secrets.yaml (age-encrypted, safe for me to
  # commit) at activation time and drops the plaintext into /run/secrets/*
  # (a tmpfs, root-only, wiped on reboot) — nothing sensitive ever touches
  # the Nix store, which is world-readable by design.
  #
  ...
  # of activation, not something Nix can generate on its own. It's a
  # one-time manual setup, same pattern as the ProtonVPN conf file used to
  # be:
  ...
  # out via config.sops.secrets."protonvpn-conf".path). It used to live as
  # a plain file at /etc/protonvpn/proton.conf, owned by me and
  # world-readable — anyone with a shell on this machine could've read the
  # tunnel's private key. Now it's root-only (0400) and only exists at all
