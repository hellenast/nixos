...
### `flake.nix`
Entry point. Pulls in nixpkgs-unstable, home-manager, nix-flatpak, caelestia-shell/cli, the caelestia-dots repo (vendored as a plain source, not a flake — pieces of it get patched/re-sourced directly in `home.nix`), zen-browser, NixVirt, sops-nix, disko (disk partitioning, see `disko.nix`), and bedrock-on-linux (Minecraft Bedrock, see `gaming.nix`). Imports `variables.nix` and spreads it into `specialArgs`/`extraSpecialArgs` for every module below.
...
### `gaming.nix`
Steam (Remote Play + dedicated-server firewall rules, gamescope session for fullscreen game launches), gamescope itself, GameMode for automatic per-game performance tweaks, Prism Launcher for Minecraft Java Edition, and bedrock-on-linux for Minecraft Bedrock Edition (the real Windows GDK build, run under steam-run/Proton with native Xbox sign-in — see "Manual setup" below for why I ended up here instead of a more "native Linux" option).

### `waydroid.nix`
A real Android container (GPU-accelerated, real Android runtime, not a reimplementation of one) for running Android apps I don't have a native Linux build of. Not currently running anything specific — I originally set this up for Minecraft Bedrock Edition, but that turned out to be broken here too (see "Manual setup" below), so Bedrock now runs through `gaming.nix` instead. Kept around anyway since it's genuinely useful for other Android apps. Uses the `waydroid-nftables` package variant, not plain `waydroid` — this kernel has no `ip_tables` module, so the default package's `iptables`-based network setup script fails outright ("Failed to setup waydroid-net" in `waydroid.log`); the nftables variant drives `nft` against `nf_tables` instead, which this kernel does support.
...
Not running anything specific right now (see Minecraft Bedrock Edition below for why), but the setup/caveats below apply to whatever Android app I eventually install in it:
- `virtualisation.waydroid.enable` handles the kernel config/LXC/dbus wiring, but everything after that is interactive, outside Nix: `sudo waydroid init -s GAPPS` once to pull down a system image (the GAPPS variant, for Play Store access — reinit with `-f` if I already ran a plain init), then `waydroid session start` and sign into a Google account inside the window that opens.
- The window doesn't resize live, and shouldn't be expected to: single-window mode (the default) renders Android to one fixed-resolution virtual display, so dragging the host window's edges just stretches/crops that fixed framebuffer instead of telling Android to re-render at the new size. Waydroid's own "multi-window" mode is meant to give each app a real, independently resizable window instead, but it's specifically broken on Hyprland — open, unresolved reports on both [Hyprland's](https://github.com/hyprwm/Hyprland/issues/6400) and [Waydroid's](https://github.com/waydroid/waydroid/issues/976) trackers describe it rendering a black window or mispositioning content. The working alternative is staying in single-window mode with an explicit fixed resolution:
  ```
  ...
  ```
  Resizing after that means re-running those `prop set` commands with different numbers and restarting the container, not dragging the window.
- If `sudo systemctl restart waydroid-container` doesn't bring it back after `waydroid status` shows `Session: RUNNING` / `Container: STOPPED`, check whether Android itself requested a reboot from inside the session (`journalctl -u waydroid-container`, look for `sys.powerctl='reboot,'`) — the LXC container has no real hardware to reboot into, so Android's own `init` shutting down just kills the container outright instead of restarting it. The restart command above is the fix either way.
- Only `/var/lib/waydroid/data` is worth backing up — that's the actual Android `/data` partition: installed apps, their data, and signed-in accounts. `/var/lib/waydroid/images` and `/var/lib/waydroid/rootfs` are just the downloaded Android system/vendor build, free to recreate with `waydroid init -s GAPPS` again, no backup needed. `waydroid.cfg` (holding the multi-window/resolution props, among other things) isn't worth backing up either — small enough to just redo. See "Reinstalling with full-disk encryption" above for the fresh-install restore order.

### Minecraft Bedrock Edition (`gaming.nix`)
Two things I tried and dropped before landing on bedrock-on-linux, in case either gets fixed upstream and becomes worth revisiting:
- `mcpelauncher-ui-qt` (an unofficial launcher that reimplements Android's dynamic linker to run Bedrock's native libraries directly on Linux) — every Bedrock version I tested segfaulted, because its custom symbol resolver is missing libc symbols (`pthread_sigmask`, confirmed present in glibc itself via `nm`/`readelf`, so the gap is in mcpelauncher's own shim) that current Bedrock builds call.
- Waydroid (a real Android container, see `waydroid.nix`) got further — the container itself runs fine — but Bedrock crashes instantly inside it with SIGSEGV in `libpairipcore.so`, Mojang's own anti-tamper library, which appears to actively refuse to run in Waydroid's userdebug/test-keys LineageOS build. Confirmed on a from-scratch reinstall (new UID, new package path, identical crash), so it isn't a corrupted install. Matches an open, unresolved upstream report: [waydroid/waydroid#2143](https://github.com/waydroid/waydroid/issues/2143), on unrelated (Intel) hardware, so it's not specific to this machine's AMD GPU either.

bedrock-on-linux (`inputs.bedrock-on-linux`, flake.nix) sidesteps both: it runs the real Windows GDK build of Bedrock under steam-run/Proton, with native Xbox sign-in — no Android layer, no custom linker, just Wine/Proton the same way any other Windows game runs under Steam. Ships no game files itself, so setup is interactive:
1. Launch `bedrock-on-linux` from the app menu (or run it once via `nix run` first if just trying it out).
2. Sign into a Microsoft account that owns Minecraft — twice: once for the Store download, once for Xbox Live (Friends/Realms/Marketplace). Achievements display in-game but won't unlock (a limitation of the tool, not this config).
3. First run downloads the game straight from Microsoft under that account — no repo/Nix involvement.
- If something breaks later (a Windows update, a crash), `bedrock-on-linux repair` rebuilds the Wine/Proton environment without touching worlds — try that before reinstalling from scratch.
- `bedrock-on-linux doctor` runs a system diagnostic if setup or launch fails.
- Data (the Proton prefix, downloaded game, worlds, Xbox sign-in) lives under `~/.local/share/bedrock-on-linux` — inside `/home`, so it's already covered by "back up everything" in "Reinstalling with full-disk encryption" above; no separate callout needed the way Waydroid's `/var/lib/waydroid/data` needed one.
